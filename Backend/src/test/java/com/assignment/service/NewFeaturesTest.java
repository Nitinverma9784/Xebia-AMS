...
public class NewFeaturesTest {
    ...
    @Test
    public void testAssignBatch_DraftWithSubmissionsClones() {
        AssignmentRepository assignmentRepository = org.mockito.Mockito.mock(AssignmentRepository.class);
        TeacherRepository teacherRepository = org.mockito.Mockito.mock(TeacherRepository.class);
        BatchRepository batchRepository = org.mockito.Mockito.mock(BatchRepository.class);
        StudentRepository studentRepository = org.mockito.Mockito.mock(StudentRepository.class);
        ...
        Batch batch = Batch.builder()
                ...
                .teacher(teacher)
                .build();
        org.mockito.Mockito.when(batchRepository.findAllById(List.of(10L))).thenReturn(List.of(batch));

        Assignment draftAssignment = Assignment.builder()
                .id(100L)
                .title("Quiz 1")
                .assignmentType(AssignmentType.QUIZ)
                .status(AssignmentStatus.DRAFT)
        ...
        org.mockito.Mockito.when(assignmentRepository.findByIdAndTeacherId(100L, 1L))
                .thenReturn(java.util.Optional.of(draftAssignment));

        // Mock submissionRepository.countByAssignmentId(100L) to return 1 (it has submissions!)
        org.mockito.Mockito.when(submissionRepository.countByAssignmentId(100L)).thenReturn(1L);

        // When saving cloned assignment, return it
        org.mockito.Mockito.when(assignmentRepository.save(org.mockito.Mockito.any(Assignment.class)))
                .thenAnswer(invocation -> invocation.getArgument(0));

        org.mockito.Mockito.when(assignmentRepository.findById(org.mockito.Mockito.anyLong()))
                .thenReturn(java.util.Optional.of(draftAssignment));

        org.mockito.Mockito.when(assignmentMapper.toResponse(org.mockito.Mockito.any(Assignment.class)))
                .thenReturn(com.assignment.dto.response.AssignmentResponse.builder()
                        .id(200L) // cloned assignment ID
                        .title("Quiz 1")
                        .status(AssignmentStatus.ACTIVE)
                        .build());

        List<com.assignment.dto.response.AssignmentResponse> responses = service.assignBatch(100L, List.of(10L), teacherEmail);
        assertNotNull(responses);
        assertEquals(1, responses.size());
        
        // The original draft assignment should still NOT have a batch (since it was cloned!)
        assertNull(draftAssignment.getBatch());
        assertEquals(AssignmentStatus.DRAFT, draftAssignment.getStatus());

        // Verify that save was called with a cloned assignment (which will have batch set to `batch`)
        org.mockito.Mockito.verify(assignmentRepository, org.mockito.Mockito.atLeastOnce()).save(org.mockito.Mockito.argThat(arg -> 
            arg != draftAssignment && arg.getBatch() == batch
        ));
    }
}
