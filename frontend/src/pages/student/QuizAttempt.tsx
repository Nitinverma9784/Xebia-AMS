    ...
    <Layout role="student" title="Quiz Attempt" subtitle={quiz.title}>
      <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 select-none animate-fade-in">
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          {/* Timer Card */}
          <Card className="bg-[#6C1D5F]/5 border-[#6C1D5F]/15 dark:bg-[#6C1D5F]/10">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider flex items-center gap-1.5">
                <Clock size={14} className="text-[#6C1D5F] dark:text-purple-400 animate-spin" /> Time Remaining
              </span>
            ...
            <div className="w-full bg-slate-200 dark:bg-slate-800 h-1.5 rounded-full mt-3 overflow-hidden">
              <div 
                className={`h-full rounded-full transition-all duration-1000 ${
                  timeLeft !== null && timeLeft < 60 ? 'bg-rose-500' : 'bg-[#6C1D5F]'
                }`}
          ...
          <Card className="border border-[var(--brand-border)]">
            ...
            <div className="w-full bg-slate-100 dark:bg-slate-800 h-2.5 rounded-full overflow-hidden mb-5">
              <div 
                className="h-full rounded-full bg-[#01AC9F] transition-all duration-300"
            ...
            <div className="grid grid-cols-5 gap-2.5">
              {questions.map((q, idx) => {
                ...
                if (isCurrent) {
                  ...
                  stateStyle = 'bg-emerald-500/15 border-emerald-500 text-emerald-600 dark:text-teal-400 font-bold';
                }
              ...
              <div className="space-y-6 flex-1 flex flex-col justify-between">
                ...
                <div className="flex justify-between items-center pb-3 border-b border-[var(--brand-border)]">
                  ...
                  <div className="flex gap-2">
                    ...
                    <span className="text-[10px] uppercase font-bold text-[#01AC9F] bg-emerald-500/10 px-2.5 py-0.5 rounded-md border border-[#01AC9F]/10 flex items-center gap-1">
                ...
                <div className="space-y-5 flex-1 py-2">
                  <h2 className="text-base font-bold text-[var(--text-primary)] leading-normal flex items-start gap-2">
                    <span className="text-[#6C1D5F] dark:text-purple-400 font-black">{currentIndex + 1}.</span>
                        ...
                        return (
                          <button
                            ...
                            className={`w-full flex items-center gap-4 p-3.5 rounded-xl border text-left text-xs font-semibold cursor-pointer transition-all duration-200 ${
                              isSelected
                                ? 'bg-[#6C1D5F]/5 border-[#6C1D5F] text-[#6C1D5F] dark:text-purple-400 font-bold shadow-sm'
                          ...
                          >
                            <span className={`w-5.5 h-5.5 rounded-full border flex items-center justify-center text-[10px] font-black shrink-0 ${
                              isSelected ? 'bg-[#6C1D5F] text-white border-transparent' : 'border-[var(--brand-border)] text-[var(--text-secondary)] bg-slate-50'
                            }`}>
                              {opt.key}
                            </span>
                  ...
                  {currentQuestion.questionType === 'SHORT_ANSWER' && (
                    <div className="space-y-2 mt-4">
                      ...
                      <input
                        ...
                        className="w-full bg-white dark:bg-[#1E293B] border border-[var(--brand-border)] focus:border-[#6C1D5F] text-[var(--text-primary)] rounded-xl py-3 px-4 text-xs transition-colors focus:outline-none"
                      />
                    </div>
                  )}
                </div>
      ...
      >
        <div className="flex items-start gap-3 select-none">
          <div className="w-10 h-10 rounded-xl bg-purple-500/10 flex items-center justify-center shrink-0">
            <CheckCircle size={20} className="text-[#6C1D5F]" />
          </div>
          <div>
